...
impl NetworkConnectionProvider for GatewayNetworkConnector {
    ...
    fn execute_transaction(
        ...
        let intent_hash_string = {
            let intent_hash = notarized_transaction
                .prepare()
                .map_err(GatewayExecutorError::NotarizedTransactionPrepareError)?
                .intent_hash();
            let transaction_hash_encoder =
                TransactionHashBech32Encoder::new(&self.network_definition);
            transaction_hash_encoder
                .encode(&intent_hash)
                .map_err(GatewayExecutorError::TransactionHashBech32mEncoderError)?
        };
                ...
                TransactionIntentStatus::CommittedSuccess => {
                    ...
                    let new_entities = {
                        ...
                        for node_id in new_global_entities {
                            let node_id = node_id?;
                            if let Ok(package_address) = PackageAddress::try_from(node_id) {
                                new_entities.new_package_addresses.insert(package_address);
                            } else if let Ok(resource_address) = ResourceAddress::try_from(node_id)
                            {
                                new_entities.new_resource_addresses.insert(resource_address);
    ...
    fn preview_transaction(
        ...
        let request = TransactionPreviewRequest {
            ...
            start_epoch_inclusive: preview_intent.intent.header.start_epoch_inclusive.number()
                as i64,
            end_epoch_exclusive: preview_intent.intent.header.end_epoch_exclusive.number() as i64,
            notary_public_key: Some(Box::new(native_public_key_to_gateway_public_key(
                &preview_intent.intent.header.notary_public_key,
            ))),
            notary_is_signatory: Some(preview_intent.intent.header.notary_is_signatory),
            ...
            flags: Box::new(TransactionPreviewRequestFlags {
                assume_all_signature_proofs: preview_intent.flags.assume_all_signature_proofs,
    ...
    fn read_component_state<V: ScryptoDecode>(
        ...
        .unwrap();
        let encoded_details = scrypto_encode(&details.to_scrypto_value()).unwrap();

        scrypto_decode(&encoded_details).map_err(GatewayExecutorError::StateScryptoDecodeError)
    }
}
...
fn native_public_key_to_gateway_public_key(
    ...
    match native_public_key {
        radix_common::prelude::PublicKey::Secp256k1(public_key) => {
            gateway_client::models::PublicKey::EcdsaSecp256k1 { key: public_key.0 }
        }
        radix_common::prelude::PublicKey::Ed25519(public_key) => {
            gateway_client::models::PublicKey::EddsaEd25519 { key: public_key.0 }
        }
    }
}
...
pub enum GatewayExecutorError {
    ...
    TransactionCommittedDetailsError(GatewayClientError<TransactionCommittedDetailsError>),
