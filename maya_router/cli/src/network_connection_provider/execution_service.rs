...
impl<'e, E: NetworkConnectionProvider> ExecutionService<'e, E> {
    ...
    > {
        ...
        let notary_is_signatory = self
            .signers_private_keys
            .iter()
            .any(|private_key| private_key.public_key() == self.notary_private_key.public_key());
        let signer_private_keys = self
            .signers_private_keys
            .iter()
            .filter(|private_key| private_key.public_key() != self.notary_private_key.public_key())
        ...
        let header = TransactionHeaderV1 {
            ...
            end_epoch_exclusive: current_epoch.after(10).expect("Not currently an issue"),
            nonce: rand::random(),
        ...
        let preview_receipt = self
            ...
            .preview_transaction(PreviewIntentV1 {
                intent: IntentV1 {
                    ...
                    blobs: BlobsV1 {
                        blobs: manifest.blobs.clone().into_values().map(BlobV1).collect(),
                    },
        ...
        if !preview_receipt.is_commit_success() {
            return Err(ExecutionServiceError::TransactionPreviewWasNotSuccessful(
                manifest.clone(),
                preview_receipt,
            ));
        }
        ...
        if insert_lock_fee {
            ...
            let total_fees_plus_padding = total_fees + self.signers_private_keys.len() * dec!(0.5);
